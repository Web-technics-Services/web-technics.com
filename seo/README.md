...
```powershell
node seo\apply.js
node seo\apply.js --check
$env:SEO_ENV = "production"
node seo\build.js build
node seo\test.js
```
...
`node seo\apply.js` to regenerate the managed part of every `<head>`, then
`node seo\build.js audit` to confirm. The codemod is idempotent: running it twice
produces no diff.
...
This property owns Cambodia local-market intent and self-canonicalizes every
route. `web-technics.services` is a sibling company property, not a mirror or
canonical target. `movedRoutes` must remain empty unless a `.com` route is
intentionally retired in the future.
