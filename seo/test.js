...
test('renderMeta emits every mandatory tag', () => {
  ...
  assert.ok(html.includes('https://web-technics.com/example.html'));
});
...
test('structured data generators produce valid schema.org nodes', () => {
  ...
  assert.strictEqual(article.url, 'https://web-technics.com/blog/storefront.html');
...
test('robots.txt blocks crawling outside production', () => {
  ...
  assert.ok(production.includes('Sitemap: https://web-technics.com/sitemap.xml'));
...
test('an empty moved-route map generates no redirects', () => {
  ...
  assert.ok(nginx.includes('# No moved routes are configured.'));
  assert.ok(!/^Redirect 301 /m.test(generateApache()));
});

test('the Cambodia property has no moved routes', () => {
  const { movedRoutes } = require('./config');
  const result = audit({ rootDir });
  ...
  assert.deepStrictEqual(movedRoutes, {});
});